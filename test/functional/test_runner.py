...
def main():
    ...
    parser.add_argument('--vpub', action='store_true', help='run Particl specific tests')
    ...
    tmpdir = "%s/vpub_test_runner_₿_🏃_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
...
def run_tests(test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0, failfast=False, runs_ci=False, create_cache=True):
    ...
    try:
        if subprocess.check_output(["pidof", "vpubd"]) is not None:
            print("%sWARNING!%s There is already a vpubd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
