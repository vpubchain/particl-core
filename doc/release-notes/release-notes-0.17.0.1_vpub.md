...
be used in V0.17 by starting vpubd with the '-deprecatedrpc=accounts'
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart vpubd with
...
- When running vpubd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If vpubd
...
- #13171 `19d8ca5` Change gitian-descriptors to use trusty instead (ken2812221)
- #13604 `75bea05` Add depends 32-bit arm support for vpub-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling vpub-qt (TheCharlatan)
...
- #12882 `d96bdd7` Make `test_vpub` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13811 `660abc1` travis: Run `bench_vpub` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in vpubd systemd service file (Flowdalic)
- #12652 `55f490a` vpub-cli: Provide a better error message when vpubd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to vpubd in debian package (ghost)
...
- #13603 `453ae5e` vpub-tx: Stricter check for valid integers (domob1812)
...
- #13714 `29b4ee6` contrib: Add lxc network setup for trusty host (ken2812221)
...
- #13725 `07ce278` Fix vpub-cli --version (Empact)
...
- #12877 `2b54155` Use vpubd in Tor documentation (knoxcard)
...
- #13905 `73a09b4` fixed vpub-cli -help output for help2man (hebasto)
